...
A guide assumes the reader already decided what to do. A use case reaches someone who has a business problem and does not yet know which products solve it. So the page carries a commercial frame. The frame is context, never salesmanship, and the line between them is thinner here than anywhere else in the corpus.
...
- No: `In today's competitive e-commerce landscape, delivering fast, seamless experiences is critical to conversion.`
- Yes: `A storefront serves a product catalogue that changes hourly and a cart that is different for every visitor. Both live under one domain. Cache helps the first and corrupts the second.`
...
```
This use case covers catalogue caching, cart bypass, and checkout protection. It does not cover payment processing or inventory synchronisation.
```

## Vertical words to technical needs

Business terms have to be translated before they can be configured. Translate to a technical need, then find the product in the corpus. Never skip to a product from memory.
...
| Personalisation | A decision made per request, before the response is built |
...
The product column is deliberately absent. It is filled per page from the evidence table in `intake.md`, by reading the corpus, because a product mapping written from memory is the exact failure this skill exists to prevent.

## Numbers

**An example figure belongs in the scenario paragraph, labelled as an example, and nowhere else.**
...
- No: `Applications handles 50,000 requests per second during a sale.`
- Yes: `A catalogue of around 50,000 products, as an example.`
...
- No: `These rules make your checkout PCI DSS compliant.`
- Yes: `The firewall filters requests to the checkout path. For Azion's own certification, see [PCI compliance](/en/documentation/platform/compliance/pci-dss-certification/).`
...
| A customer name, a logo, a case study | This is a public repository, and docs are not references |
| A competitor comparison | A use case is not a migration guide |
| An industry statistic | It ages, it is unsourceable here, and it changes nothing |
| `powerful`, `seamless`, `robust`, `enterprise-grade` | Quality claims. `.agents/references/writing-quality.md` |
| A closing paragraph that restates the page | End on the last check or on a link worth following |

Naming a competitor is legitimate in the migration guides under `src/content/docs/en/pages/guides/`, which exist to move a reader off a named platform. That exemption does not extend to a use case.
...
- [ ] No percentage, price, customer, competitor, or industry statistic
