...
Thirteen page kinds. Pick by what the reader is trying to do, then follow the kind's skeleton exactly — the opening move, the section order, and the closing are mandated, not suggestions. That is what keeps a hundred pages by a hundred authors reading as one site.
...
| Look up what a term means | [Glossary](#glossary) | Reference |
...
Look-up definitions for the vocabulary one product uses. Base form Reference: descriptive register, 25-word cap.

- **Tone**: neutral, lexicographic.
- **Title**: `Glossary`; Portuguese `Glossário`.
- **Opening move**: one sentence naming the product whose terms the page defines. Then the table. No history, no pitch.
- **Body**: one GFM table, `| Term | Definition |`, inside `<GlossaryFilter lang="...">` so readers can filter it — mechanics in `.agents/references/components.md`. Rows sort alphabetically by term, per language. Blank lines around the table inside the component are mandatory.
- **A term is lowercase** unless it is a product name or another proper noun. Terms from the never-translate list stay in English on the Portuguese page.
- **A definition is one to three sentences.** The first sentence says what the term is. Link the canonical page inline from the phrase it explains. A term that needs more than three sentences needs a concept page, and the definition links to it.
- **Every definition traces to a page or a source you can name.** The glossary records how the documentation uses a term; it never defines a term no page uses.
- **Closing**: none. The table is the page.
- Each row's term is a deep-linkable anchor (`#cache-key`), generated by the component from the term itself.
...
| Changelog, Navigation hub, Glossary | none |
...
| Glossary | The canonical page for each term, inline from its definition |
...
This file is the condensed skeleton set. Refer to https://5yye2daquza.map.azionedge.net/en/documentation/style-guide/content/choose-a-content-type/ for the router, which links a full page for each of the thirteen kinds.
