...
description: >-
  ...
  Orchestrator.
permalink: /documentacao/deploy/orchestrator/guides/trabalhar-com-variaveis/
menu_namespace: orchestratorMenu
meta_tags: >-
...
Durante o processo de implantação no [Orchestrator](/pt-br/documentacao/deploy/orchestrator/), você pode trabalhar com variáveis. Uma variável pode ser definida em [edge services](#variaveis-em-edge-services) e [edge nodes](#variaveis-em-edge-nodes).

Você pode criar variáveis de edge node e de edge services usando:

- [Azion Console](/pt-br/documentacao/account/guides/como-acessar-o-azion-console/)
...
1. Acesse o [Azion Console](/pt-br/documentacao/account/guides/como-acessar-o-azion-console/).
...
3. Selecione o [edge service](/pt-br/documentacao/deploy/orchestrator/guides/criar-edge-service/) ao qual você deseja adicionar uma variável.
...
1. Execute a seguinte requisição `GET` no seu terminal, substituindo [TOKEN VALUE] pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) e recupere o ID do edge service ao qual você deseja adicionar uma variável:
...
2. Crie uma variável relacionada ao serviço. Execute a seguinte requisição `PATCH` no seu terminal, substituindo `[TOKEN VALUE]` por seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/), `:id` pelo ID do service e informe o nome e o valor da variável:
...
Substitua `[TOKEN VALUE]` por seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/), `:id` pelo ID do edge service e informe os seguintes parâmetros no corpo da requisição:
...
5. Escolha o [serviço](/pt-br/documentacao/deploy/orchestrator/guides/criar-edge-service/) ao qual você deseja aplicar a variável.
...
1. Acesse o [Azion Console](/pt-br/documentacao/account/guides/como-acessar-o-azion-console/) > **Edge Services**.
...
<Fragment slot="panel.api">
1. Execute a seguinte requisição `GET` no seu terminal, substituindo [TOKEN VALUE] por seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) e recupere o ID do edge node ao qual você deseja adicionar uma variável:
...
2. Execute a seguinte requisição `GET` no seu terminal, substituindo [TOKEN VALUE] por seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) e `:id` pelo ID do edge node, e recupere o ID da relação entre o edge node e o serviço escolhido:
...
Substitua [TOKEN VALUE] por seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/), `:id` pelo ID do edge node e `:bind_id` pelo ID da relação recuperada na etapa 2, informando as seguintes propriedades no corpo da requisição:
