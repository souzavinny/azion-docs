...
```mermaid
flowchart TD
  Client([Cliente]) -->|Requisição HTTP| Node[Edge node recebe a requisição]
  Node --> FwRule{Regra de firewall com Run Function}
  FwRule -->|Critérios correspondem| FwHandler[O handler firewall é executado no Azion Runtime]
  FwHandler -->|"ctx.deny() chamado"| Blocked([Requisição bloqueada])
  FwHandler -->|Retorna sem deny| AppRule{Regra de aplicação com Run Function}
  FwRule -->|Sem correspondência| AppRule
  AppRule -->|Critérios correspondem| Fetch[O handler fetch é executado no Azion Runtime]
  AppRule -->|Sem correspondência| NoFn([Nenhuma função é executada])
  Fetch --> Response([Resposta retornada ao cliente])
```
...
Rules Engine da aplicação então aplica suas próprias regras, e uma regra **Run Function** cujos critérios correspondem executa o handler `fetch` da instância selecionada. Esse handler retorna uma `Response` ao cliente.
...
```mermaid
flowchart LR
  Project[Projeto de framework] --> Bundler[Azion Bundler]
  Bundler --> Fn[Funções geradas em JavaScript ou WebAssembly]
  Fn --> Runtime[Azion Runtime]
  Runtime --> DC[Data centers da Azion]
```
