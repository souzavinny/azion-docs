...
- **Tiered Cache**: limpa o cache da camada tiered cache no edge. Requer que o módulo [Tiered Cache](/pt-br/documentacao/build/applications/cache/tiered-cache/) esteja ativo.
...
Limpa objetos em cache através de uma lista de [cache keys](/pt-br/documentacao/build/applications/cache/#cache-keys) como argumento. 
...
- Formato de imagem de acordo com o suporte do navegador ao usar [Image Processor](/pt-br/documentacao/build/applications/image-processor/).
...
Você pode fazer o purge de objetos com variação de conteúdo baseada em cookies ou query strings usando a funcionalidade **Advanced Cache Key** do módulo [Application Accelerator](/pt-br/documentacao/build/applications/application-accelerator/). 
...
Para remover objetos com [variação baseada em cookies](/pt-br/documentacao/build/applications/cache/#advanced-cache-key), você pode usar a purge por cache key, inserindo todas as variações individualmente, ou uma purge por wildcard, usando `@@*` no final.

Para remover objetos com [variação com base na query string](/pt-br/documentacao/build/applications/cache/#advanced-cache-key), você pode usar: 
...
Você pode remover imagens que são tratadas pelo [Image Processor](/pt-br/documentacao/build/applications/image-processor/).
...
Por padrão, a Azion permite o cache apenas dos métodos `GET` e `HEAD`. Você pode ativar o cache de `POST` ou `OPTIONS` com o módulo [Application Accelerator](/pt-br/documentacao/build/applications/application-accelerator/#support-for-http-methods).
