...
permalink: /documentacao/build/applications/reference/real-time-purge/
---
...
- **Tiered Cache**: limpa o cache da camada tiered cache no edge. Requer que o módulo [Tiered Cache](/pt-br/documentacao/build/applications/reference/cache/tiered-cache/) esteja ativo.
...
Limpa objetos em cache através de uma lista de [cache keys](/pt-br/documentacao/build/applications/reference/cache/#cache-keys) como argumento. 
...
- Cookies ao usar [Advanced Cache Key](/pt-br/documentacao/build/applications/reference/cache-settings/#advanced-cache-key).
- Formato de imagem de acordo com o suporte do navegador ao usar [Image Processor](/pt-br/documentacao/build/applications/reference/image-processor/).
...
Você pode fazer o purge de objetos com variação de conteúdo baseada em cookies ou query strings usando a funcionalidade **Advanced Cache Key** do módulo [Application Accelerator](/pt-br/documentacao/build/applications/reference/application-accelerator/). 
...
Para remover objetos com [variação baseada em cookies](/pt-br/documentacao/build/applications/reference/cache/#advanced-cache-key), você pode usar a purge por cache key, inserindo todas as variações individualmente, ou uma purge por wildcard, usando `@@*` no final.

Para remover objetos com [variação com base na query string](/pt-br/documentacao/build/applications/reference/cache/#advanced-cache-key), você pode usar: 
...
Você pode remover imagens que são tratadas pelo [Image Processor](/pt-br/documentacao/build/applications/reference/image-processor/).
...
Se você usar o recurso [Large File Optimization](/pt-br/documentacao/build/applications/reference/cache-settings/#large-file-optimization) em sua aplicação, arquivos grandes ainda poderão permanecer no cache. Para realizar o purge nesse caso, você deve listar as cache keys de cada fatia individualmente.
...
Por padrão, a Azion permite o cache apenas dos métodos `GET` e `HEAD`. Você pode ativar o cache de `POST` ou `OPTIONS` com o módulo [Application Accelerator](/pt-br/documentacao/build/applications/reference/application-accelerator/#support-for-http-methods).
