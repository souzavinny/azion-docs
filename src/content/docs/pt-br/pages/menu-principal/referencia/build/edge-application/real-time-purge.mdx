...
Porém, se objetos específicos armazenados em cache precisarem ser removidos do cache antes de expirarem, você tem a opção de limpar o cache com o Real-Time Purge. A limpeza regular do cache ajuda você a:
...
- **Cache**: limpa o cache mantido nos edge nodes da Azion.
- **Tiered Cache**: limpa o cache da segunda camada, entre os edge nodes e a origem. Requer que o módulo [Tiered Cache](/pt-br/documentacao/build/cache/tiered-cache/) esteja ativo.
...
- Cookies ao usar [Advanced Cache Key](/pt-br/documentacao/build/cache/reference/cache-settings/#advanced-cache-key).
...
Se você usar o recurso [Large File Optimization](/pt-br/documentacao/build/cache/reference/cache-settings/#large-file-optimization) em sua aplicação, arquivos grandes ainda poderão permanecer no cache. Para realizar o purge nesse caso, você deve listar as cache keys de cada fatia individualmente.
...
Os limites do Real-Time Purge, incluindo as operações de purge que variam por plano de suporte, estão listados em [Limites do Cache](/pt-br/documentacao/build/cache/limites/).
