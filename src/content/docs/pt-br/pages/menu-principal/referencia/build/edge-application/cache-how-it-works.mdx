...
```mermaid
flowchart TD
  A[Cliente envia uma requisição] --> B[Rules Engine executa a fase de request]
  B --> C{Alguma cache key corresponde no edge node?}
  C -- Correspondência encontrada --> D[Edge node entrega o objeto a partir do cache]
  C -- Sem correspondência --> E[Edge node busca o objeto no servidor de origem]
  E --> F[Edge node armazena o objeto e gera sua cache key]
  D --> G[Rules Engine executa a fase de response]
  F --> G
  G --> H[Edge node entrega a resposta ao cliente]
```
...
```mermaid
flowchart TD
  A[Objeto no edge node expira] --> B[Edge node revalida com o servidor de origem]
  B -- Revalidação bem-sucedida --> C[Edge node armazena e entrega o objeto novo]
  B -- Revalidação falha --> D{A janela de stale continua aberta?}
  D -- Janela aberta --> E[Edge node entrega a cópia expirada e atualiza em segundo plano]
  D -- Janela fechada --> F[Edge node não consegue responder a requisição a partir do cache]
```
