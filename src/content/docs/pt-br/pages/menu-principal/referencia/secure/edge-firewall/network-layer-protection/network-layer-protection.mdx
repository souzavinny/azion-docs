...
| Escopo | Fonte  |
| ------ | ------ |
| Criar network lists | [Como criar listas de bloqueio/permissão de IP, ASN e geolocalização com Network Lists](/pt-br/documentacao/secure/network-shield/guides/blocklists-enderecos-ip-edge/) |
| Bloquear redes Tor | [Como bloquear endereços IP de exit nodes Tor](/pt-br/documentacao/secure/firewall/guides/bloquear-redes-tor/) |
...
:::tip[dica]
Saiba mais sobre as configurações de [Firewall](/pt-br/documentacao/secure/firewall/) e [Rules Engine para Firewall](/pt-br/documentacao/secure/firewall/reference/rules-engine/).
:::
...
:::note[importante]
As criteria e behaviors disponíveis na aba Rules Engine dependem de quais módulos de Firewall estão ativados. Saiba mais sobre os [módulos do Firewall](/pt-br/documentacao/secure/firewall/).
:::

A configuração de Network Shield é feita na aba **Rules Engine** do Firewall. Na aba, você pode usar *criteria* de `Network` para criar regras com [Network Lists](/pt-br/documentacao/secure/firewall/reference/network-shield/network-lists/), com base na rede, geolocalização do usuário ou ASN. É possível utilizar listas prontas, mantidas e atualizadas pela própria Azion. Você também pode monitorar e bloquear comportamentos suspeitos, ou até mesmo aplicar restrições de acordo com o *behavior* escolhido.
...
```text
product: Network Shield
former_name: Network Layer Protection
type: módulo de Firewall
criteria: Hostname, Network, Request URI, Scheme
behaviors: Deny (403 Forbidden), Drop (Close Without Response), Set Rate Limit
list_types: IP/CIDR, país (geolocalização), ASN, listas mantidas pela Azion (ex.: Azion IP Tor Exit Nodes)
