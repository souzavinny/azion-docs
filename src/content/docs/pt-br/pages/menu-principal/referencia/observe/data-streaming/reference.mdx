---
title: Referência do Data Stream
description: >-
  Referência completa do Data Stream: data sources e suas variáveis, templates,
  domínios, campos dos conectores de endpoint, configurações de payload e
  números de entrega de logs.
meta_tags: >-
  data, streaming, edge computing, stream, observe, observability,
  observabilidade, logs, events, eventos, referência, variáveis, endpoints
namespace: documentation_observe_data_stream_reference
permalink: /documentacao/observe/data-stream/reference/
---

import LinkButton from 'azion-webkit/linkbutton'

Esta página documenta cada seção de configuração do [Data Stream](/pt-br/documentacao/observe/data-stream/): data sources e suas variáveis, templates, domínios, endpoints e seus campos, configurações de payload e números de entrega de logs. Para explicações conceituais sobre agrupamento, tratamento de erros e disponibilidade de endpoint, veja os [Conceitos do Data Stream](/pt-br/documentacao/observe/data-stream/concepts/).
...
Por exemplo, se um bloco atinge 2.000 registros em 13 segundos, ele é enviado.

O tempo para os dados serem transferidos para a infraestrutura do cliente é de *até 3 minutos*.
...
O data source **Activity History** exibe os dados referentes aos [registros de atividade](/pt-br/documentacao/account/reference/activity-history/) de sua conta no Azion Console. As seguintes variáveis estão disponíveis para essa opção:
...
O data source **Applications** exibe os dados das requisições feitas para suas [applications](/pt-br/documentacao/build/applications/) na Azion. As seguintes variáveis estão disponíveis para essa opção:
...
| $upstream_addr | Endereço IP e porta do cliente. Também pode armazenar múltiplos servidores ou grupos de servidores. Exemplo: 192.168.1.1:80. Quando a resposta é `127.0.0.1:1666`, o upstream é o Azion [Cells Runtime](/pt-br/documentacao/build/functions/runtime-apis/overview/). |
...
:::note
Você pode adicionar a variável `$traceback` em um custom template com o data source Applications se você tem ativada a opção de *Debug rules* em sua aplicação. Veja mais em [Debugging rules em Applications](/pt-br/documentacao/build/applications/reference/rules-engine/#debugging-rules).
:::

As variáveis: `$upstream_bytes_received`; `$upstream_cache_status`; `$upstream_connect_time`; `$upstream_header_time`; `$upstream_response_time` e `$upstream_status` podem apresentar *mais de um elemento separado por vírgula*. Quando uma conexão é disparada, seja por redirecionamento interno ou escolha de origem com [Load Balancer](/pt-br/documentacao/build/connectors/reference/load-balancer/), por exemplo, cada valor contido no campo representa a respectiva conexão iniciada. O campo pode ser separado por:
...
Requer o produto **Functions**.

O data source **Functions** exibe os dados referentes às solicitações feitas para suas [functions](/pt-br/documentacao/build/applications/reference/functions/) na Azion.
...
Requer o produto **Web Application Firewall**.
...
<LinkButton severity="secondary" link="/pt-br/documentacao/observe/data-stream/guides/data-stream-template-personalizado/" label="Como criar um template personalizado" />

---

## Domínios

Você pode associar os seus [domínios existentes](/pt-br/documentacao/build/applications/reference/domains/) registrados na Azion ao seu stream. Se você ainda não tem um domínio registrado na sua conta, veja a documentação sobre [Criar um novo domínio associado a sua application](/pt-br/documentacao/build/workloads/guides/configurar-dominio/).

Ao associar um domínio, os eventos relacionados a esse ou a esses domínios específicos são coletados e enviados para o endpoint que você configura por meio de um Data Stream. Você pode associar um ou mais domínios e você tem a opção de filtrar domínios com **Filter Domains** ou selecionar todos os domínios com **All Domains**.
...
<LinkButton severity="secondary" link="/pt-br/documentacao/observe/data-stream/guides/data-stream-associar-dominios/" label="Como associar domínios" />

Se você selecionar a opção **All Domains**, você também pode configurar a porcentagem de dados que quer receber, de forma aleatória, do seu stream através da opção de *Sampling*.
...
Quando a opção de Sampling está habilitada, você só pode adicionar *um* stream em sua conta. Após esse stream ser desabilitado, a opção **Add Streaming** é habilitada novamente na tela do Data Stream no Azion Console. Veja os [Conceitos do Data Stream](/pt-br/documentacao/observe/data-stream/concepts/#sampling) para entender como o sampling funciona.
...
Se você deseja usar esse protocolo, certifique-se de que o endpoint que irá receber os dados esteja devidamente protegido com um certificado digital emitido por uma *Certificate Authority (CA)* reconhecida globalmente, como IndenTrust, DigiCert, Sectigo, GoDaddy, GlobalSign ou Let’s Encrypt.
...
- **Stream Name**: refere-se ao nome do stream que o usuário definiu ao criar o Kinesis Data Firehose. Exemplo: `MyKDFConnector`
...
<LinkButton severity="secondary" link="/pt-br/documentacao/observe/data-stream/guides/amazon-kinesis-endpoint/" label="Como utilizar o AWS Kinesis Data Firehose para receber dados" />
...
<LinkButton severity="secondary" link="/pt-br/documentacao/observe/data-stream/guides/azure-blob-endpoint/" label="Como utilizar o Azure Blob Storage para receber dados" />
...
<LinkButton severity="secondary" link="/pt-br/documentacao/observe/data-stream/guides/azure-monitor-endpoint/" label="Como utilizar o Azure Monitor para receber dados" />
...
- **Datadog URL**: a URL ou a URI do seu endpoint no Datadog. Exemplo: `http-intake.logs.datadoghq.com`
- **API Key**: chave de API gerada no dashboard do Datadog. Exemplo: `your-datadog-api-key-here`

<LinkButton severity="secondary" link="/pt-br/documentacao/observe/data-stream/guides/datadog-endpoint/" label="Como utilizar o Datadog para receber dados" />
...
<LinkButton severity="secondary" link="/pt-br/documentacao/observe/data-stream/guides/elasticsearch-endpoint/" label="Como utilizar o Elasticsearch para receber dados" />
...
<LinkButton severity="secondary" link="/pt-br/documentacao/observe/data-stream/guides/google-bigquery-endpoint/" label="Como utilizar o Google BigQuery para receber dados" />
...
- **Secret Key**: chave secreta para acesso ao bucket fornecida pelo seu provedor. Exemplo: `your-secret-access-key`
...
<LinkButton severity="secondary" link="/pt-br/documentacao/observe/data-stream/guides/amazon-s3-endpoint/" label="Como utilizar o Amazon S3 para receber dados" />
<LinkButton severity="secondary" link="/pt-br/documentacao/observe/data-stream/guides/conector-azion-object-storage/" label="Como utilizar o Object Storage da Azion para receber dados" />
...
<LinkButton severity="secondary" link="/pt-br/documentacao/observe/data-stream/guides/splunk-endpoint/" label="Como utilizar o Splunk para receber dados" />
...
Para configurar o endpoint Standard HTTP/HTTPS POST, você deve obter as seguintes informações:
...
Requer o endpoint **Standard HTTP/HTTPS POST**.
...
Por padrão, o Data Stream recomenda o formato NDJSON com o uso de `\n` como um log line separator e `$dataset` como payload format, que usa as informações fornecidas no campo de código **Data Set**, descrevendo as variáveis escolhidas como template. Veja os [Conceitos do Data Stream](/pt-br/documentacao/observe/data-stream/concepts/#formatos-ndjson-e-json) para entender a diferença entre NDJSON e JSON.
...
Você pode personalizar o payload com informações específicas se você está usando o data source [Activity History](#activity-history) e o endpoint [Standard HTTP/HTTPS POST](#standard-httphttps-post).
