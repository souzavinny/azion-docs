...
permalink: /documentacao/store/object-storage/guides/criar-bucket/
meta_tags: >-
...
Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) e `name` pelo nome desejado do bucket para criar um novo bucket somente leitura:
...
Execute a seguinte requisição `PATCH` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) e `bucket_name` pelo nome do bucket ao qual você deseja atribuir a permissão de leitura e escrita:
