...
1. Execute a seguinte requisição `PATCH` no seu terminal, substituindo `[VALOR DO TOKEN]` pelo seu [token pessoal](/pt-br/documentacao/account/guides/personal-tokens/) e a variável `<edge_firewall_id>` pelo [ID do seu firewall](https://api.azion.com/v4#/operations/GetEdgeFirewallFirewalls)) para ativar o módulo **Functions**:
...
:::tip
Para obter mais informações sobre o endpoint de Rules Engine, consulte [Trabalhar com rules engine](/pt-br/documentacao/secure/firewall/guides/trabalhar-com-rules-engine/).
:::
