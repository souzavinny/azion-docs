...
permalink: /documentacao/secure/firewall/guides/mtls/
...
O [Mutual Transport Layer Security (mTLS)](/pt-br/documentacao/secure/certificate-manager/reference/mtls/) é um protocolo de segurança que permite autenticação e criptografia de duas vias para interações de rede. Ele confirma que tanto o cliente quanto o servidor estão autenticados e têm autorização para comunicação efetiva.

Para configurar o mTLS em suas aplicações, você precisa de um [digital certificate](/pt-br/documentacao/secure/firewall/guides/certificado-digital/) com suporte a mTLS assinado por uma autoridade certificadora, ou Certificate Authority (CA). Na Azion, este tipo de certificado é chamado de **Trusted CA**.

Existem instruções separadas para API v3 usando configurações de [Domains legadas](/pt-br/documentacao/build/applications/reference/domains/) e API v4 usando o novo produto [Workloads](/pt-br/documentacao/secure/workloads/).

:::tip
Neste guia, há instruções separadas para API v3 e API v4. Se você não tem certeza de quais etapas se aplicam à sua conta, consulte [o guia Verificar Migração da Conta](/pt-br/documentacao/account/guides/verificar-migracao-conta/) para determinar se sua conta já foi migrada.
:::
...
1. Acesse o [Azion Console](/pt-br/documentacao/account/guides/como-acessar-o-azion-console/) > **Workloads**.
...
1. Acesse o [Azion Console](/pt-br/documentacao/account/guides/como-acessar-o-azion-console/) > **Domains**.
...
1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [token pessoal](/pt-br/documentacao/account/guides/personal-tokens/) e o certificado na propriedade `certificate`:
...
4. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [token pessoal](/pt-br/documentacao/account/guides/personal-tokens/), o `<application_id>` por um ID de application, e o `<digital_certificate_id>` pelo ID do Trusted CA que você criou:
...
| `edge_application_id` | O [ID da aplicação](/pt-br/documentacao/build/applications/guides/criar-uma-aplicacao/) vinculada ao domínio |
...
6. Execute a seguinte requisição `POST` no seu terminal para vincular seu novo workload a uma application, substituindo `[TOKEN VALUE]` pelo seu [token pessoal](/pt-br/documentacao/account/guides/personal-tokens/), o `<application_id>` por um ID de application, e o `<workload_id>` pelo ID do workload que você criou:
...
1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) e o certificado na propriedade `certificate`:
...
4. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/), a `<application_id>` por um ID de application, e `<digital_certificate_id>` pelo ID do **Trusted CA** que você criou:
...
1. Acesse o [Azion Console](/pt-br/documentacao/account/guides/como-acessar-o-azion-console/) > **Firewall**.
2. [Configure um firewall](/pt-br/documentacao/secure/firewall/guides/firewall-definir-main-settings/) para o domínio mTLS.
...
1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) e o `<edge_firewall_id>` pelo ID do firewall associado ao domínio mTLS:
...
Para especificar variáveis mTLS em cabeçalhos HTTP, você deve usar o [Rules Engine para Application](/pt-br/documentacao/build/applications/reference/rules-engine/). O exemplo a seguir descreve um conjunto de regras que adicionará a variável `${ssl_client_escaped_cert}` ao cabeçalho da sua aplicação.

Requer:
- <Tag severity="info" client:only="vue">[Application Accelerator](/pt-br/documentacao/build/applications/guides/definir-configuracoes-principais/#modulos)</Tag>
- <Tag severity="info" client:only="vue">[Dominio com certificado mTLS](#adicione-um-trusted-ca-e-associe-a-um-dominio)</Tag>

:::tipcs
Você também pode inserir cabeçalhos com outras variáveis da [lista de variáveis mTLS disponíveis](/pt-br/documentacao/build/applications/reference/rules-engine/#variaveis-mtls).
:::
...
<Fragment slot="panel.console">
1. Acesse o [Azion Console](/pt-br/documentacao/account/guides/como-acessar-o-azion-console/) > **Application**.
...
<Fragment slot="panel.api">
1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) e `<application_id>` com o ID da application associada ao domínio com mTLS:
