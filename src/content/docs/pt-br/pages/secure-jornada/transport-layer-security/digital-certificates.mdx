...
permalink: /documentacao/secure/firewall/guides/certificado-digital/
...
[Certificate Manager](/pt-br/documentacao/secure/certificate-manager/) são essenciais para fornecer à sua infraestrutura confiabilidade e conformidade com as regulamentações de proteção de dados. Na Azion, você tem várias opções para proteger seus domínios com certificados TLS. Além de [registrar um certificado e chave privada](/pt-br/documentacao/secure/workloads/guides/criar-certificado-digital/) que você já possui, você também pode [solicitar um certificado Let's Encrypt](/pt-br/documentacao/secure/certificate-manager/guides/como-gerar-um-certificado-lets-encrypt/) para o seu domínio.
...
:::note
Certifique-se de que o name e os SANs listados sejam iguais aos [domínios que você configurou com a Azion](/pt-br/documentacao/secure/workloads/guides/configurar-dominio/). Isso evitará que ocorram erros quando você associar o certificado ao domínio.
:::
...
1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) e os placeholder com os dados desejados:
...
<Fragment slot="panel.api">
1. Execute a seguinte requisição `PATCH` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/), o `<digital_certificate_id>` pelo ID recebido na resposta anterior e o certificado na propriedade `certificate_content`: 
