...
1. Execute a seguinte requisição `GET` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) para buscar seu `<zone_id>`:
...
4. Execute a seguinte requisição `PATCH` para ativar o DNSSEC na zona e iniciar a geração das chaves criptográficas. Substitua `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) e `{zone_id}` pelo ID coletado no passo anterior:
...
6. Execute a seguinte requisição `GET` para recuperar os valores de `digest` e `key_tag` gerados. Substitua `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) e `{zone_id}`. Você vai precisar deles para configurar o DNSSEC no seu Registrador de domínios.
