---
title: "Acesse IPs por meio de um root domain (ANAME)"
description: >-
  Configure o seu Edge DNS para permitir o acesso a um endereço por meio de seu
  root domain.
meta_tags: 'secure, Edge DNS, domain, naked domain, root domain, aname, entry, records'
namespace: docs_guides_secure_idns_root_domain
permalink: /documentacao/secure/firewall/guides/acessar-root-domain/
menu_namespace: firewallMenu
---

import LinkButton from 'azion-webkit/linkbutton'
import Tabs from '~/components/tabs/Tabs'
import Code from '~/components/Code/Code.astro'


Você pode configurar o seu **Edge DNS** para permitir o acesso a um endereço por meio de seu root domain, também conhecido como *naked domain* ou *apex domain*, usando um registro **ANAME**.

Para este guia, considere que você está configurando um registro para uma zona com o root domain `domain.com`.

<LinkButton link="/pt-br/documentacao/secure/firewall/guides/adicionar-registros/" label="saiba mais sobre adicionar registros" severity="secondary" />

---

<Tabs client:visible>
    <Fragment slot="tab.console">Console</Fragment>
    <Fragment slot="tab.api">API</Fragment>

<Fragment slot="panel.console">
1. Siga as etapas gerais descritas no guia [Como adicionar registros em uma zona de DNS](/pt-br/documentacao/secure/firewall/guides/adicionar-registros/).
2. Em **Name**, insira `@`. Dessa forma, você utilizará seu root domain: `domain.com`.
3. No menu suspenso **Record Type**, selecione **ANAME**.
4. Em **Value**, adicione os endereços IP que você deseja disponibilizar por meio do root domain. Eles devem estar no formato FQDN. Exemplo: `32082s.map.azionedge.net`.
5. Escolha o tipo de **Policy**: **Simple** ou **Weighted**.
6. Clique no botão **Save**.
</Fragment>


<Fragment slot="panel.api">
1. Execute a seguinte requisição `GET` no seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) para obter seu `<hosted_zone_id>`:

<Code lang="bash" code={`
curl --request GET \
  --url https://api.azion.com/v4/edge_dns/zones \
  --header 'Accept: application/json' \
  --header 'Authorization: Token [TOKEN VALUE]'
`} />

2. Você receberá uma resposta com todas as suas zonas existentes. Copie o valor do `<id>` que você deseja usar.
3. Execute a seguinte requisição `POST`, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) e o valor `<hosted_zone_id>` que você copiou:

<Code lang="bash" code={`
curl --request POST \
  --url https://api.azion.com/v4/edge_dns/zones/<zone_id>/records \
  --header 'Accept: application/json' \
  --header 'Authorization: Token [TOKEN VALUE]' \
  --header 'Content-Type: application/json' \
  --data '{
    "type": "ANAME",
    "name": "@",
    "rdata": ["xxxxx.map.azionedge.net"],
    "ttl": 20
  }'

`} />


4. Você receberá uma resposta semelhante a esta:

<Code lang="json" code={`
{
  "results": {
    "answers_list": [
      "32082s.map.azionedge.net"
    ],
    "zone_id": 1234,
    "record_type": "ANAME",
    "ttl": 0,
    "policy": "simple",
    "entry": "",
    "id": 12345,
    "description": ""
  },
  "schema_version": 3
}
`} />

Aguarde alguns minutos para que as alterações se propaguem e seus registros serão criados na zona que você escolheu.

:::tip
Consulte a [documentação da API da Azion](https://api.azion.com/) e para obter mais informações sobre o que a API da Azion pode oferecer.
:::
</Fragment>

</Tabs>
---

