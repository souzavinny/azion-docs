...
permalink: /documentacao/secure/firewall/guides/criar-waf-rule-set/
...
1. Execute a seguinte requisição `POST` para criar uma regra, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/):
...
3. Execute a seguinte requisição `POST` para criar uma regra, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/), a variável `<edge_firewall_id>` com o [ID do firewall](https://api.azion.com/#015cb4a3-8fa2-4625-9792-2cc271e184eb) e `<waf_rule_set_id>` com o ID que você copiou:
...
:::note
O comportamento `set_waf` neste payload referencia o conjunto de regras WAF apenas por meio do atributo `waf_id`. O modo do WAF (**Learning** ou **Blocking**) é atualmente configurado via Azion Console, como mostrado na aba **Console** deste guia — o payload de API acima não inclui um campo de modo. Consulte a [referência do WAF](/pt-br/documentacao/secure/waf/) para as capacidades atuais.
:::
