...
permalink: /documentacao/account/guides/conditional-access-by-ip-address/
...
1. Execute a seguinte requisição `POST` no seu terminal, substituindo `[Token]` pelo seu [personal token](/en/documentation/account/guides/personal-tokens/) para configurar sua política.
...
1. Execute a seguinte requisição `GET` no seu terminal, substituindo `[Token]` pelo seu [personal token](/en/documentation/account/guides/personal-tokens/) para listar todas as suas políticas de conta configuradas.
...
4. Execute a seguinte requisição `PUT` no seu terminal, substituindo `[Token]` pelo seu [personal token](/en/documentation/account/guides/personal-tokens/) e `<id>` pelo identificador que você copiou na etapa anterior, para atualizar o status da sua política.
...
4. Execute a seguinte requisição `DELETE` no seu terminal, substituindo `[Token]` pelo seu [personal token](/en/documentation/account/guides/personal-tokens/) e `<id>` pelo identificador que você copiou na etapa anterior, para excluir a política.
