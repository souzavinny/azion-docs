...
- **Custom Host Header**: em alguns casos, pode ser necessário substituir o valor do cabeçalho `Host` na requisição para a origem. Defina um valor para o cabeçalho `Host` neste campo.
    - Use a variável `${host}` para retirar o valor do cabeçalho `Host` da URI da requisição. Saiba mais sobre o Host Header e como customizá-lo no [documentação de Origins](/pt-br/documentacao/secure/connectors/reference/origins/#host-header).
