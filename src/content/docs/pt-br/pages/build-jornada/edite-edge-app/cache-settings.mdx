...
permalink: /documentacao/build/applications/guides/ajustar-cache-settings/
---

import Tabs from '~/components/tabs/Tabs'


Utilize as capacidades do [Cache](/pt-br/documentacao/build/applications/reference/cache-settings/) para configurar as [políticas de cache](/pt-br/documentacao/build/applications/reference/cache-settings/) da sua application, incluindo valores de expiração no edge ou no navegador, otimização de arquivos grandes para dividir conteúdos em partes menores e segmentação de cache key com base em métodos HTTP, query strings, cookies ou grupos de dispositivos.

:::caution[atenção]
Algumas funcionalidades de cache requerem que o módulo [Application Accelerator](/pt-br/documentacao/build/applications/reference/application-accelerator/) esteja ativo. Se o **Application Accelerator** estiver ativado, a transferência de dados pode gerar custos relacionados ao uso. Consulte a [página de preços](/pt-br/documentacao/platform/pricing/) para mais informações.
:::

---

Quando você [cria uma application](/pt-br/documentacao/get-started/comecar-com-um-template/) pela primeira vez, uma variável de configuração de cache será criada e ativada por padrão. Este guia mostrará como criar e ativar uma nova instância de configuração de cache.
...
:::note
Os valores máximo e padrão de TTL para o edge devem estar entre `60` e `31536000` segundos (1 ano). Se você tiver o módulo [Application Accelerator](/pt-br/documentacao/build/applications/reference/application-accelerator/) ativado, você pode definir este valor para um mínimo de `0`. Além disso, se você tiver o módulo [Cache](/pt-br/documentacao/build/applications/reference/cache/tiered-cache/) ativado e desejar manter o cache na camada de Cache, você deve definir o TTL para pelo menos `3` segundos.

Um TTL zero não é o mesmo que um cache bypass. Para mais detalhes, leia sobre a diferença entre essas configurações na documentação [behavior Bypass Cache](/pt-br/documentacao/build/applications/reference/rules-engine/#bypass-cache).
:::
...
Em vez de baixar um arquivo de conteúdo grande e correr o risco de timeouts ou terminações de conexão, [os arquivos podem ser divididos](/pt-br/documentacao/build/applications/reference/cache-settings/#large-file-optimization) em partes menores e armazenados em cache sob demanda.
...
Esta seção permite que você personalize como o cache varia com base em métodos HTTP, campos de query string, cookies e grupos de dispositivos por meio de [cache keys](/pt-br/documentacao/build/applications/reference/cache-settings/#advanced-cache-key). Você pode escolher segmentar as cache keys usando esses atributos para controlar como os objetos são armazenados e servidos a partir do cache.

:::caution[atenção]
Para habilitar todas as funcionalidades do **Application Accelerator**, você deve ativar o módulo [Application Accelerator](/pt-br/documentacao/build/applications/reference/application-accelerator/).
:::
...
Para analisar como seu conteúdo está sendo armazenado em cache, você pode verificar os indicadores de cache da aplicação usando o [Modheader para Google Chrome](/pt-br/documentacao/build/applications/guides/verificar-tempo-de-cache-da-pagina/).
</Fragment>

<Fragment slot="panel.api">
1. Execute a seguinte requisição `PATCH` em seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/) e a variável `<application_id>` pelo [ID da sua application](/pt-br/documentacao/build/applications/guides/definir-configuracoes-principais/) para ativar o módulo [Application Accelerator](/pt-br/documentacao/build/applications/reference/application-accelerator/):
...
3. Se você quiser configurar a entrega adaptativa para um dos seus [device groups](/pt-br/documentacao/build/applications/guides/criar-device-groups/), execute a seguinte requisição `GET` antes:
...
5. Execute a seguinte requisição `POST` em seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/), a variável `<application_id>` pelo [ID da sua application](/pt-br/documentacao/build/applications/guides/definir-configuracoes-principais/), e a variável `<device_group_id>` pelo ID do device group da resposta anterior, se necessário:
...
```
  ...
  | `modules.edge_cache.stale_cache.enabled` | Habilita o [stale cache](/pt-br/documentacao/build/applications/reference/cache-settings/#stale-cache) para servir conteúdo expirado temporariamente enquanto busca conteúdo atualizado na origem. |
  | `modules.edge_cache.large_file_cache.enabled` | Habilita a [Otimização de arquivos grandes](/pt-br/documentacao/build/applications/reference/cache-settings/#large-file-optimization) ao armazenar arquivos grandes em fragmentos. |
...
7. Execute a seguinte requisição `POST` em seu terminal, substituindo `[TOKEN VALUE]` pelo seu [personal token](/pt-br/documentacao/account/guides/personal-tokens/), a variável `<application_id>` pelo [ID da sua application](/pt-br/documentacao/build/applications/guides/definir-configuracoes-principais/), e a variável `<cache_setting_id>` pelo ID da configuração de cache recebido na resposta:
...
Para verificar como seu conteúdo está sendo armazenado em cache, você pode [verificar os indicadores de cache da aplicação usando o Modheader para Google Chrome](/pt-br/documentacao/build/applications/guides/verificar-tempo-de-cache-da-pagina/).
