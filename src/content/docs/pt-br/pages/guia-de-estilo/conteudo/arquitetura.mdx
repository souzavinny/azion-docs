...
O diagrama mostra o caminho de uma requisição e de sua resposta em uma aplicação na rede distribuída da Azion. A requisição do cliente chega a um nó da Azion, onde o Rules Engine e a camada de cache a processam. O conteúdo que não está em cache vem do servidor de origem, e a resposta retorna ao cliente pelo mesmo nó.
...
2. No nó da Azion, o Rules Engine processa a requisição. Ele aplica políticas de cache e comportamentos de otimização de imagem na fase de request.
3. A camada de cache avalia a requisição. Em uma correspondência de cache key, o nó entrega o objeto a partir do cache.
4. Em um cache miss, o nó encaminha a requisição ao servidor de origem. A origem responde, e o nó armazena o conteúdo em cache.
5. Antes de a resposta retornar ao cliente, o Rules Engine processa as políticas da fase de response. O nó então entrega o conteúdo ao usuário.
````
