...
Quando o conjunto de limites cresce o bastante para ser consultado sozinho, ele vai para o slot Limites da seção de produto. As regras abaixo não mudam: só o lugar muda. [Arquitetura de informação](/pt-br/documentacao/guia-de-estilo/conteudo/arquitetura-de-informacao/) tem a lista de slots.
...
- **Nunca invente um limite nem um nome de plano.** Os dois vêm de uma fonte nomeada: a página de pricing, o contrato de planos ou o time de produto. Uma célula não confirmada recebe um marcador `[GAP: ...]` e é publicada como lacuna.
...
Este exemplo mostra uma tabela de limites dimensionada por plano, e como fica um valor não confirmado:
...
| --- | --- | --- | --- |
| Buckets | [GAP: plan breakdown not confirmed] | 100 per account | Yes |
| S3 credential access keys | [GAP: plan breakdown not confirmed] | 100,000 per account | Yes |
```

Os marcadores de lacuna são o ponto. A fonte desses números declara um padrão único, sem separação por plano. A coluna de plano é publicada como lacuna, nunca como valor inventado. Quem revisa vê exatamente o que falta.
