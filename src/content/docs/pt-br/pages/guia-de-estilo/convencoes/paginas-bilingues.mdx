...
Quando uma tarefa entrega só a página em português, abra a página em inglês antes de escrever. Se ela não existe, pare e diga isso: uma página em português escrita primeiro não tem fonte, e a página em inglês que vier depois vira uma tradução de uma tradução.
...
- Não traduza termos técnicos genéricos: `data center`, `serverless`, `template`, `compliance`, `on-premise`. As strings `edge`, `edge computing` e `edge location` permanecem sem tradução onde já aparecem, mas textos novos não as introduzem.
...
As listas completas estão em [Terminologia da documentação](/pt-br/documentacao/guia-de-estilo/escrita/terminologia/).
