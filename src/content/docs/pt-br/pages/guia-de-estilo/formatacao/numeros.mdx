...
description: >-
  Como escrever números na documentação da Azion: quando escrever por extenso,
...
`MB` e `MiB` são números diferentes. Escreva a unidade que a fonte declara e nunca converta entre as formas decimais (`kB`, `MB`, `GB`) e binárias (`KiB`, `MiB`, `GiB`). O mesmo vale para taxas: `Gbps` conta bits e `MB/s` conta bytes, oito vezes de diferença, então mantenha a forma que a fonte usa.
