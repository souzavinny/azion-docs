...
description: >-
  As regras de escrita que mantêm uma página utilizável com um leitor de tela:
  títulos e links descritivos, sem linguagem direcional, texto alternativo e
...
Alguns leitores usam um leitor de tela, alguns navegam pelo teclado e alguns não distinguem cores. As regras desta página mantêm a documentação utilizável para todos eles. Elas seguem as Web Content Accessibility Guidelines (WCAG) 2.1.
...
- Correto: `Clique em **Salvar**.`
- Incorreto: `Veja a seção acima.`
- Correto: `Veja a seção Pré-requisitos.`
...
O primeiro uso de uma sigla em cada página escreve o termo por extenso e coloca a sigla entre parênteses: `time to live (TTL)`. O leitor chega a qualquer página diretamente, por um resultado de busca, e uma definição em outra página não existe para ele. A regra completa, com a diferença para nomes de produto, está em [Escolha de palavras](/pt-br/documentacao/guia-de-estilo/escrita/escolha-de-palavras/).
