...
description: >-
  Como a documentação da Azion aplica o ASD-STE100: limites de frase por tipo de
  conteúdo, tempos simples, voz ativa e as regras que evitam ambiguidade.
...
## Dê uma ação por passo

Em um procedimento, cada passo carrega exatamente uma ação. Quando um passo contém duas, o leitor muitas vezes completa a primeira e pula a segunda.

- Antes: `Crie o bucket e envie seus arquivos, depois configure as permissões.`
- Depois: três passos numerados, um para cada ação: `Crie o bucket.` `Envie seus arquivos.` `Configure as permissões.`

[Procedimentos](/pt-br/documentacao/guia-de-estilo/escrita/procedimentos/) é o dono desta regra, incluindo o único caso em que dois movimentos pequenos dividem um passo e uma única frase.
...
- Depois: `O Firewall bloqueia a requisição.`

## Use a voz ativa

Escreva cada passo na voz ativa. Em texto descritivo, use a passiva apenas quando o agente é desconhecido ou é a própria plataforma. Quando um produto age, prefira nomeá-lo como sujeito. A voz ativa também mantém o registro em segunda pessoa descrito em [Voz da documentação](/pt-br/documentacao/guia-de-estilo/escrita/voz/).

- Antes: `A requisição é bloqueada quando uma regra corresponde.`
- Depois: `O Firewall bloqueia a requisição quando uma regra corresponde.`
...
## Não use contrações em inglês

Em inglês, escreva a forma completa de cada verbo: "do not", não "don't". Uma contração soa informal e dificulta a tradução. As páginas deste guia seguem esta regra.

- Antes: `You don't need to purge the cache.`
- Depois: `You do not need to purge the cache.`
...
As regras de divisão de página estão em [Tamanho de página](/pt-br/documentacao/guia-de-estilo/conteudo/tamanho-de-pagina/).
...
## Trate os limites como metas

Os limites de 20 palavras em procedimento e 25 em descrição são metas. Nenhuma verificação no CI conta palavras. Uma frase descritiva clara com 26 palavras não é um defeito. Reescreva uma frase quando o comprimento esconde o significado, não para satisfazer um número.

## Confira seu rascunho

Confirme cada ponto antes de entregar um rascunho:

- Cada frase de procedimento tem no máximo 20 palavras, e cada frase descritiva tem no máximo 25.
- Cada passo dá exatamente uma instrução.
- Cada tempo verbal é simples, e o comportamento do produto usa o presente simples.
- Cada passo está na voz ativa, e as descrições usam a passiva apenas quando o agente é desconhecido ou é a própria plataforma.
...
- Cada verbo em inglês aparece na forma completa, sem contração.
