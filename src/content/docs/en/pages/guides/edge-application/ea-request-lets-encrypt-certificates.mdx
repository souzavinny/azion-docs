...
permalink: /documentation/build/applications/guides/how-to-generate-a-lets-encrypt-certificate/
---
...
Web applications that use the HTTPS protocol require a [Certificate Manager](/en/documentation/build/applications/reference/domains/mtls/). When you redirect your traffic to Azion, you have the option to generate a *Let's Encrypt*™ certificate, which is a free and secure way to encrypt data for your application. Azion automates the issuance, renewal, and deactivation of this TLS certificate through an internal certificate management solution.
...
:::note
There is also the possibility to [create a Let's Encrypt certificate via Azion CLI](/en/documentation/devtools/cli/reference/create/#digital-certificate-id).
:::
...
Below are steps for adding a certificate to your application. There are separate instructions for the [legacy Domains](/en/documentation/build/applications/reference/domains/) settings and the new [Workloads](/en/documentation/secure/workloads/) product.

:::tip
If you're not sure which steps apply to your account, see [the Verify Your Account Migration to API v4 guide](/en/documentation/account/guides/verify-account-migration/) to determine if your account has already been migrated.
:::
...
To create a **Workload**, you must have an application. If you haven't created an application yet, check the [Build an application documentation](/en/documentation/get-started/journeys/launch/).

Once you have an application, you need to [create a Workload](/en/documentation/secure/workloads/) and list the CNAMEs that you want to secure with the Let's Encrypt certificate. To do so, follow these steps:
...
To create an Azion **Domain**, you must have an application. If you haven't created an application yet, check the [build an application](/en/documentation/get-started/journeys/launch/) documentation.

Once you have an application, you need to [create a domain](/en/documentation/build/applications/reference/domains/) and list the CNAMEs that you want to secure with the Let's Encrypt certificate. To do so, follow these steps:
...
:::tip
For more information on issuing Let's Encrypt certificates, visit the [Certificate Manager for Secure reference](/en/documentation/secure/certificate-manager/#lets-encrypt-certificate).
:::
...
Below are steps for pointing your traffic to Azion. There are separate instructions for the [legacy Domains](/en/documentation/build/applications/reference/domains/) settings and the new [Workloads](/en/documentation/secure/workloads/) product.
...
Note that there might be a delay in propagation time when you access your application. If that's the case, you can [run the DIG command](/en/documentation/secure/edge-dns/guides/run-the-dig-command/) in your terminal to check whether your domain points to the Azion address.
