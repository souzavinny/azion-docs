...
permalink: /documentation/build/applications/guides/original-ip-header/
menu_namespace: applicationsMenu
---

When a user makes a request to an application, the `X-Forwarded-For` HTTP header stores the client IP. However, since [edge nodes](/en/documentation/build/applications/) mediate requests to an application, the header also stores other addresses in the request route in addition to the client IP address.

To isolate the client IP, you must forward it to a new header by creating a rule using [Rules Engine](/en/documentation/build/applications/reference/rules-engine/) for **Applications**.
