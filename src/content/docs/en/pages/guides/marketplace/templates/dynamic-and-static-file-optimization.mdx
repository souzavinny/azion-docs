...
- **Custom Host Header**: in some cases, it may be necessary to override the value of the `Host` header request in the request to retrieve information from the origin. Set a value for the `Host` header in this field.
    - Use the `${host}` variable to retrieve the Host value from the request URI. Read more about the Host Header and how to customize it on the [Origins documentation](/en/documentation/secure/connectors/reference/origins/#host-header).
