...
permalink: /documentation/deploy/orchestrator/guides/work-with-variables/
...
During the deployment process on [Orchestrator](/en/documentation/deploy/orchestrator/), you can work with variables. A variable can be set on an [edge service](#edge-services-variables) and an [edge node](#edge-node-variables).

You can create edge node and edge services variables using:

- [Azion Console](/en/documentation/account/guides/how-to-access-azion-console/)
...
1. Go to [Azion Console](/en/documentation/account/guides/how-to-access-azion-console/).
2. On the top-left corner, select the **Products menu**, represented by three horizontal lines, and then **Edge Services**.
3. Select the [edge service](/en/documentation/deploy/orchestrator/guides/create-edge-service/) to which you want to add a variable.
...
1. Run the following `GET` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and retrieve the edge service ID to which you wish to add a variable:
...
2. Create a variable related to the service. Run the following `PATCH` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and informing the variable's name and value:
...
Replace `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/), `:id` with the ID of the edge service and inform the following parameters in the request body:
...
5. Choose the [service](/en/documentation/deploy/orchestrator/guides/create-edge-service/) to which you want to apply the variable.
...
1. Go to [Azion Console](/en/documentation/account/guides/how-to-access-azion-console/) > **Edge Services**.
...
<Fragment slot="panel.api">
1. Run the following `GET` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and retrieve the edge node ID to which you wish to add a variable:
...
2. Run the following `GET` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and `:id` with the ID of the edge node, and retrieve the ID of the relation between the edge node and the chosen service:
...
Replace `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/), `:id` with the ID of the edge node and `:bind_id` with the ID of the relation retrieved in step 2, informing the following properties in the request body:
