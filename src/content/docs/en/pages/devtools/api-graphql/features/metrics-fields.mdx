...
Each [GraphQL API dataset](/en/documentation/devtools/graphql-api/features/#datasets) has specific fields that can be used when querying for data from that specific dataset. These fields are also used for the [Real-Time Metrics](/en/documentation/observe/real-time-metrics/) interface on Azion Console.
