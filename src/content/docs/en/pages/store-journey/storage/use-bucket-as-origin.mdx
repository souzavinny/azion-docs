...
permalink: /documentation/store/object-storage/guides/use-bucket-as-origin/
...
This guide walks you through setting a bucket as origin of an applications using the [Azion API](https://api.azion.com/), [Azion CLI](/en/documentation/devtools/cli/), and [Azion Runtime](/en/documentation/build/functions/runtime-apis/storage/).
...
There are separate instructions for API v3 using [legacy Origin](/en/documentation/build/applications/reference/domains/) settings and API v4 using the new [Connector](/en/documentation/products/secure/connectors) product.
...
3. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and writing the desired bucket name in `name` to create a read-only bucket:
...
To facilitate this process, use the [Applications Proxy](/en/documentation/marketplace/integrations/edge-application-proxy-template/) template:
...
1. [Access Azion Console](/en/documentation/account/guides/how-to-access-azion-console/) > **Connectors**.
...
1. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/):
...
4. Run the following `POST` request in your terminal to create an Connector, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/)
...
7. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/)
...
10. Run the following `POST` request to bind the Workload to your Applications, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/), `<edge_application_id>` with the ID of the applications and `<workload_id>` with the id of the Workload.
...
3. Copy the value from the `edge_application_id` field and paste it in a text editing app to save it for later requests.
4. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and `<edge_application_id>` with the ID of the applications you created:
...
7. Run the following `POST` request to create an object storage origin for the application, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/), `<edge_application_id>` with the ID of the applications, and the value of `bucket` with the name of the bucket you created:
...
- [A personal token configured](/en/documentation/devtools/cli/reference/globals/#token).
...
:::tip
After creating your application with an Object Storage bucket as the origin, you can manage and configure settings, such as [cache settings](/en/documentation/build/applications/guides/cache-settings/).
:::
