...
permalink: /documentation/store/object-storage/guides/create-bucket/
meta_tags: >-
...
Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and `name` with the desired bucket name to create a new read-only bucket:
...
Run the following `PATCH` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and `bucket_name` with the name of the bucket to attribute the read-write permission:
