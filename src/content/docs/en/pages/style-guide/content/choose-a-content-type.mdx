...
A slot is a position in a product section; a kind is the shape of a page. The [information architecture](/en/documentation/style-guide/content/information-architecture/) lists thirteen slots, and this catalog holds twelve kinds. The two lists are not the same length, and they never need to be.

Several slots share one kind. Limits, Examples, and Features and capabilities all hold reference pages, placed in different slots because readers reach them at different moments. Best practices and How it works both hold concept pages. Adding a slot never adds a kind, and a page that fits no kind is a page whose content type is still undecided.
