...
| 11 | Pricing | Yes | A link to the product's section of the pricing page |
| 12 | Changelog | Yes | A link to the product's changelog entries |

Two slots have a fixed sidebar shape, and it is not a dropdown.
...
**Reference pages sit directly in the sidebar, one row each.** A reader consulting a field name should reach it in one click, so reference never collapses behind a parent row. When a product has more reference pages than the sidebar can carry flat, that is a signal to merge pages, not to nest them.
...
- **The twelve never grow in number.**
