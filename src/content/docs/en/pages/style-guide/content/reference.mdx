...
When the limits set grows large enough to consult on its own, it moves into its own Limits slot in the product section. The rules below do not change: only the location does. [Information architecture](/en/documentation/style-guide/content/information-architecture/) holds the slot list.
...
- **Never invent a limit or a plan name.** Both trace to a named source: the pricing page, the plans agreement, or the product team. An unconfirmed cell gets a `[GAP: ...]` marker and ships as a gap.
...
This excerpt shows a limits table dimensioned by plan, and what an unconfirmed value looks like:
...
The gap markers are the point. The source for these numbers states a single default, with no breakdown by plan. The plan column therefore ships as a gap, never as an invented value. A reviewer sees exactly what is missing.
