...
- Write at least one Get started page per product, as the entry point directly after the product Overview.
...
A Get started page follows the tutorial skeleton. [Tutorials](/en/documentation/style-guide/content/tutorials/) argues each part; the list below names what the pattern requires. A single page takes the title `Get started with <Product>`. An interface variant takes `Get started with <Product> using <interface>`: `using Azion Console`, `using the Azion CLI`, `using the API`, or `using an AI agent`.
...
- **Split by interface when the interface changes the path.** One page is the floor. Add a variant when an interface has its own first-run audience and its own steps: Azion Console, the Azion CLI, the API, or an AI agent connected through the [Azion MCP server](/en/documentation/agent-setup/). The variants sit under one Get started group in the sidebar, Console first, and the group is a menu parent, not a page.
- **Name the sibling paths in one line**, under the opening move: `Prefer the CLI? Refer to [Get started with <Product> using the Azion CLI](/path/).` The reader who lands on the wrong path leaves it in one step.
- **Delete a variant you cannot keep true.** Every variant multiplies the commands that run before a release, and a stale first-run page fails the reader at first contact.
