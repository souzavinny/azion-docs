...
Descriptive: 25 words per sentence, active voice, passive only where the actor is genuinely unknown or is the platform itself. [Sentence structure](/en/documentation/style-guide/writing/sentence-structure/) holds the rules.
...
- **Opening move**: a declarative statement of how the system behaves. Never `This page explains`.
...
- **A diagram**: when the relationship is easier to draw than to describe. Alt text describes what the diagram shows.
...
- **Mirror the roadmap.** One noun-phrase `##` per mechanism, in the order the roadmap sentence names them.
...
- **Make diagrams carry load.** Alt text describes what the diagram shows; assets are root-absolute with no language prefix.
...
- **Mark gaps instead of inventing.** A mandated slot the input does not fill gets a `[GAP: ...]` marker, never silent omission and never invention.
