...
The sections keep this order. The interfaces list — Azion Console, API, Azion CLI, Terraform — is optional: it is true of nearly every Azion product, so include it only when an interface is distinctive, and let the router carry it otherwise.
