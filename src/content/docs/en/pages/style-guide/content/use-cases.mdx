...
title: Use cases
description: >-
  How to write a use-case page: a business scenario turned into an executable
  specification, with required products, an architecture, and measurable results.
...
The sections keep this order. Each one has a budget, because a specification that nobody can hold in view stops being usable:

| Section | Budget |
| --- | --- |
| Scenario | 500 characters |
| Prerequisites | 300 |
| Required products | 900 |
| Reference architecture | 1,200 |
| Configure sections, four at most | 4,800 |
...
That totals about 10,400 characters. A use case is the one kind that may pass the 8,000-character page target in [Page size and retrieval](/en/documentation/style-guide/content/page-size/): aim at 11,000 and keep the 16,000 hard cap. Aim the English at 10,000 so the Portuguese pair fits.
...
- **Build the requirements table before writing any prose.** Every row names its source. A row you cannot source is a row you cut, or a documentation gap you report.
...
- **Never invent a product name, a field, or a value.** A URL or a directory path is not evidence.
...
| Requirement | Technical need | Product | Documented at |
| --- | --- | --- | --- |
| Catalogue pages served fast under load | Cache identical responses at Azion's distributed network | Applications | [Cache settings](/en/documentation/build/cache/reference/cache-settings/) |
