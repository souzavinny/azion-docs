...
description: >-
  When bold and italics apply, how internal links and asset paths are formatted,
  when a list is numbered, and how to write reference tables.
...
Bold does its job when it points at the interface: `Click **Save** to apply the change.` The reader knows which word to look for.

## Use italics only to define a term

Italics mark a term at the moment the page defines it: `A *cache key* identifies one object in the cache.` After the definition, the term appears in plain text. Italics anywhere else dilute that signal, so the documentation uses them rarely.
