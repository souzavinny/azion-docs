...
description: >-
  How to write numbers in Azion documentation: when to spell them out,
...
`MB` and `MiB` are different numbers. Write the unit the source states and never convert between decimal (`kB`, `MB`, `GB`) and binary (`KiB`, `MiB`, `GiB`) forms. The same applies to rates: `Gbps` counts bits and `MB/s` counts bytes, eight times apart, so keep the form the source uses.
...
Capitalize `AM` and `PM` and put a space before them: `10:45 AM`.
