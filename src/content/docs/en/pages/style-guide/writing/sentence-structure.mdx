...
In a procedure, each step carries exactly one action. When a step contains two, readers often complete the first and skip the second. [Procedures](/en/documentation/style-guide/writing/procedures/) owns this rule, including the one case where two small movements share a step and a single sentence.
...
Write every step in the active voice. In descriptive text, use the passive voice only when the actor is unknown or is the platform itself. When a product acts, prefer naming it as the subject. [Documentation voice](/en/documentation/style-guide/writing/voice/) owns this rule and the reason: the active voice names the actor.
...
Write the full form of each verb: "do not", not "don't". [Documentation voice](/en/documentation/style-guide/writing/voice/) owns this rule.
