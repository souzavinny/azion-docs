...
In a procedure, each step carries exactly one action. When a step contains two, readers often complete the first and skip the second.

- Before: `Create the bucket and upload your files, then set the permissions.`
- After: three numbered steps, one for each action: `Create the bucket.` `Upload your files.` `Set the permissions.`

[Procedures](/en/documentation/style-guide/writing/procedures/) owns this rule, including the one case where two small movements share a step and a single sentence.
