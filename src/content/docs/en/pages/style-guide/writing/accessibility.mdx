...
title: Accessibility
description: >-
  The writing rules that keep a page usable with a screen reader: descriptive
  titles and links, no directional language, alt text, and explicit
  instructions.
...
Some readers use a screen reader, some navigate with the keyboard, and some do not distinguish colors. The rules on this page keep the documentation usable for all of them. They follow the Web Content Accessibility Guidelines (WCAG) 2.1.
...
- Correct: `See the Prerequisites section.`
...
- Correct: `![Diagram of a request that flows from the client through the edge to the origin](/assets/docs/images/uploads/request-flow.png)`
...
The first use of an acronym on every page spells out the term and puts the acronym in parentheses: `time to live (TTL)`. A reader lands on any page directly, from a search result, so a definition on another page does not exist for them. The full rule, with the difference for product names, is in [Word choice](/en/documentation/style-guide/writing/word-choice/).
