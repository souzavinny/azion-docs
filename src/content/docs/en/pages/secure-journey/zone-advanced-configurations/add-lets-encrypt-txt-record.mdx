...
permalink: /documentation/secure/firewall/guides/lets-encrypt-record/
...
If you've chosen Azion's **Domain** feature to create a Let's Encrypt certificate directly from Azion's platform for a zone that is fully configured in Edge DNS, the entire process is automated, including the handling of the TXT record. Find out more in the [Let's Encrypt guide](/en/documentation/secure/certificate-manager/guides/lets-encrypt/).
:::
...
1. Follow the general steps described in the [How to add records](/en/documentation/secure/firewall/guides/add-records/) guide.
...
1. Run the following `GET` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) to retrieve your `<hosted_zone_id>`:
...
3. Run the following `POST` request, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and the `<zone_id>` value you copied:
