...
permalink: /documentation/secure/firewall/guides/work-with-rules-engine/
...
Implementing rules in your firewall using [Rules Engine](/en/documentation/secure/firewall/reference/rules-engine/) will allow you to determine the rules it should execute in specific scenarios.
...
:::note
There are different criteria that require different Firewall modules. Make sure you have the [right module](/en/documentation/secure/firewall/reference/rules-engine/#criteria) activated to create your rules.
:::
...
1. Run the following `POST` request to create a rule, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and the `<edge_firewall_id>` variable with your firewall id value:
...
:::tip
If you need to forward the authenticated client's identity to your origin server using mTLS — for example, passing the client certificate Common Name (CN) via the `client_cn` header — use [Rules Engine for Applications](/en/documentation/build/applications/reference/rules-engine/) with the `${ssl_client_s_dn_parsed}` variable and the **Add Request Header** behavior. See [Specifying mTLS variables in HTTP headers](/en/documentation/secure/firewall/guides/mtls/#specifying-mtls-variables-in-http-headers) for a complete guide.
:::
