...
permalink: /documentation/secure/firewall/guides/mtls/
...
[Mutual Transport Layer Security (mTLS)](/en/documentation/secure/certificate-manager/reference/mtls/) is a security protocol that enables two-way authentication and encryption for network interaction. It confirms that both the client and the server are authenticated and have the clearance for effective communication.

To configure mTLS in your applications, you need a [digital certificate](/en/documentation/secure/firewall/guides/certificate-manager/) with mTLS support signed by a third-party certificate authority (CA). At Azion, this type of certificate is called a **Trusted CA**.

There are separate instructions for API v3 using [legacy Domains](/en/documentation/build/applications/reference/domains/) settings and API v4 using the new [Workloads](/en/documentation/secure/workloads/) product.

:::tip
In this guide, there are separate instructions for API v3 and API v4. If you're not sure which steps apply to your account, see [the Verify Your Account Migration guide](/en/documentation/account/guides/verify-account-migration/) to determine if your account has already been migrated.
:::
...
1. Access [Azion Console](/en/documentation/account/guides/how-to-access-azion-console/) > **Workloads**.
...
1. Access [Azion Console](/en/documentation/account/guides/how-to-access-azion-console/) > **Domains**.
...
1. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and the certificate in the `certificate` property:
...
4. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/), the `<application_id>` with an application ID, and the `<digital_certificate_id>` with the ID of the Trusted CA you created:
...
| `edge_application_id` | The [ID of the application](/en/documentation/get-started/journeys/launch/) linked to the domain |
...
6. Run the following `POST` request in your terminal to bind your new workload to an application, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/), the `<application_id>` with an application ID, and the `<workload_id>` with the ID of the workload you created:
...
1. Access [Azion Console](/en/documentation/account/guides/how-to-access-azion-console/) > **Firewall**.
2. [Configure an firewall](/en/documentation/secure/firewall/guides/firewall-configure-main-settings/) for the mTLS domain.
...
1. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and the `<edge_firewall_id>` with the ID of the firewall associated with the mTLS domain:
...
If your application is part of the Open Banking model, you must specify the variables `${ssl_client_escaped_cert}` and `${ssl_client_s_dn_parsed}` in the HTTP headers of your application. You can use the [Rules Engine for Application](/en/documentation/build/applications/reference/rules-engine/) to create this behavior for your whole application.

Requires:

- <Tag severity="info" client:only="vue">[Application Accelerator](/en/documentation/build/applications/guides/configure-main-settings/#modules)</Tag>
...
:::tip
You can also insert headers with other variables from [the list of available mTLS variables](/en/documentation/build/applications/reference/rules-engine/#mutual-transport-layer-security-mtls-variables).
:::
...
1. Access [Azion Console](/en/documentation/account/guides/how-to-access-azion-console/) > **Application**.
...
<Fragment slot="panel.api">
1. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and the `<application_id>` with the ID of the application associated with the mTLS domain:
