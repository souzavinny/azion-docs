...
permalink: /documentation/secure/firewall/guides/certificate-manager/
...
[Certificate Manager](/en/documentation/secure/certificate-manager/) are essential to provide your infrastructure with reliability and compliance with data protection regulations. At Azion, you have several options to secure your domains with TLS certificates. Aside from [registering a certificate and private key](/en/documentation/secure/workloads/guides/create-a-digital-certificate/) that you already own, you can also [request a Let's Encrypt certificate](/en/documentation/secure/certificate-manager/guides/lets-encrypt/) for your domain.
...
:::note
Ensure that the subject name and the subject alternate names (SAN) listed are the same as the [domains you configure with Azion](/en/documentation/secure/workloads/guides/configure-a-domain/). This will avoid errors from occurring when you associate the certificate with the domain.
:::
...
1. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and the placeholders in the body with the desired values: 
...
Once you receive a certificate issued by your CA, you must associate it with the CSR entry you created. After your certificate is active with Azion, you may [associate the certificate to a domain](/en/documentation/secure/workloads/guides/create-a-digital-certificate/#associate-certificate-to-domain).
...
<Fragment slot="panel.api">
1. Run the following `PATCH` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/), the `<digital_certificate_id>` with the ID you got in the previous response and the certificate in the `certificate_content` key: 
