...
permalink: /documentation/secure/firewall/guides/create-waf-rule-set/
...
1. Run the following `POST` request to create a rule, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/):
...
:::note
The `set_waf` behavior in this payload references the WAF rule set through the `waf_id` attribute only. The WAF mode (**Learning** or **Blocking**) is currently configured via Azion Console, as shown in the **Console** tab of this guide — the API payload above doesn't carry a mode field. See the [WAF reference](/en/documentation/secure/waf/) for current capabilities.
:::
