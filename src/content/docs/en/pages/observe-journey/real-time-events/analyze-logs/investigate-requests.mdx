...
permalink: /documentation/observe/data-stream/guides/investigate-requests-graphql-api/
...
As you've identified a country you don't want to allow access to is accessing your application, you can create a [network list](/en/documentation/secure/firewall/reference/network-shield/network-lists/) to block the country.
...
<LinkButton link="/en/documentation/secure/network-shield/guides/blocklists-ip-addresses-edge/#associate-network-list-with-edge-firewall-rule-set" label="go to associate network list to rule guide" severity="secondary" target="_blank" />
...
1. [Access Console](/en/documentation/account/guides/how-to-access-azion-console/) > **Firewall**.
2. Select an existing firewall or [create a new one](/en/documentation/secure/firewall/guides/firewall-configure-main-settings/).
...
:::tip
If you notice there's legitimate requests from other countries being blocked by the `User-Agent` header rule, use [WAF Tuning](/en/documentation/secure/firewall/guides/tune-waf/) to analyze requests and create allowed rules.
:::
...
If you don't want to block all requests from a country through a network list, there are other features of Azion [Secure products](/en/documentation/secure/firewall/) you can use.

- Create a rule with the [Set Rate Limit behavior](/en/documentation/secure/firewall/reference/rules-engine/#behaviors).
