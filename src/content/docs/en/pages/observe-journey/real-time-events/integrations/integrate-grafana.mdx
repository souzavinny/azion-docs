---
title: How to integrate Azion with Grafana
description: >-
  Integrate with the Azion data source plugin on Grafana to create and visualize
  charts and tables.
meta_tags: 'azion, edge, observe, observability, logs, grafana, analytics'
namespace: docs_integrate_grafana
permalink: /documentation/observe/real-time-metrics/guides/integrate-grafana/
menu_namespace: realTimeMetricsMenu
---

The Azion data source plugin on Grafana allows you to visualize the data from your existing applications on Azion on a Grafana dashboard with charts and tables. It queries data from [Real-Time Metrics](/en/documentation/observe/real-time-metrics/) and [Real-Time Events](/en/documentation/observe/real-time-events/), which use the [GraphQL APIs](/en/documentation/devtools/graphql-api/overview/).

---

## Requirements

To use the Azion data source plugin, you need:

- An [Azion account](https://console.azion.com/signup).
- A [personal token](/en/documentation/account/reference/personal-tokens/) to authenticate your account.
- One or more [applications](/en/documentation/build/applications/) created on your account.
- [Access to Grafana](https://grafana.com/).

---

## Installing the plugin on Grafana

Currently, to use the Azion data source plugin, you must install it locally and enable your account to use unsigned plugins. See the documentation on [the plugin's GitHub repository](https://github.com/aziontech/grafana-plugin/blob/dev/README.md#install-azion-plugin-on-local-grafana-install-locally) for a step-by-step on how to install it.

:::note
Azion focuses on an on-premise approach. This plugin is only available with local instances.
:::
<br /><br />

