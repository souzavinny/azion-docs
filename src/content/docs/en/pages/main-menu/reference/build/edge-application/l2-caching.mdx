---
title: Tiered Cache
description: >-
  Tiered Cache establishes an extra cache layer between the edge nodes and your origin
  servers, effectively lowering latency and minimizing infrastructure expenses.
meta_tags: >-
  caching, edge, cache, edge computing, l2, layer 2, cache layer, purge
  cache, tiered cache
namespace: documentation_products_edge_application_l2_caching
menu_namespace: cacheMenu
permalink: /documentation/build/cache/tiered-cache/
---

import LinkButton from 'azion-webkit/linkbutton'

**Tiered Cache** is an **Cache** module that creates an additional cache layer between the edge nodes and your origin servers, reducing latency and infrastructure costs.

When accessing your applications on Azion, your end-user accesses Azion's distributed network, where your content is cached. By enabling Tiered Cache in Applications, you can:

- Cache at a second layer, between the edge nodes and your origin servers.
- Keep your content cached for longer periods of time.
- Select the best region to keep cache closer to your users.

:::note
Tiered Cache is only available if the service has been activated for your account. To activate **Tiered Cache**, [contact the Sales team](https://www.azion.com/en/contact-sales/).
:::

## Implementation

| Scope | Resource |
| --- | --- |
| Cache module | [Cache](/en/documentation/build/cache/)  |
| Creating a cache setting | [How to configure cache policies for Applications](/en/documentation/build/cache/guides/cache-settings/) |

---

## Recommended TTL

**Tiered Cache** is specially designed for objects that can remain in cache for a long period of time. Thus, you can only activate it on cache policies with a TTL equal to or greater than *3 seconds*, with the default TTL being *60 seconds*.

<LinkButton link="/en/documentation/build/cache/reference/cache-settings/#browser-cache-settings" label="Learn more about expiration settings" />

---

## Regions

When you enable **Tiered Cache** as an extra caching layer, the edge servers of this second caching layer are hosted in the US (`na-united-states`) by default. 

Azion also offers the option of hosting the Tiered Cache servers in Brazil (`sa-brazil`). To switch between regions in your applications, please contact the [Sales team](https://www.azion.com/en/contact-sales/).

---

## Real-Time Purge

Whenever necessary, you can use **Real-Time Purge** to expire your **Tiered Cache** content before the expected TTL time. Remember to purge first from the tiered cache layer and only later from **Cache**. This avoids edge feedback with outdated Tiered Cache content.

<LinkButton link="/en/documentation/build/cache/reference/real-time-purge/" label="Learn more about Real-Time Purge" />

---

## Limits

The Tiered Cache minimum and maximum TTL, and the bypass cache caveat, are listed in [Cache limits](/en/documentation/build/cache/limits/).
