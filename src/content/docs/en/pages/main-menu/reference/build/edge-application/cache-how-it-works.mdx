...
flowchart LR
  A[Client request] --> B{Cache key match?}
  B -- Match --> C[Serve from cache]
  B -- No match --> D[Fetch from origin]
  D --> E[Store object and key]
  E --> C
```
...
%%{init: {'look':'neo','theme':'base','themeVariables':{'primaryBorderColor':'#F3652B'}}}%%
flowchart LR
  A[Object expires] --> B[Revalidate with origin]
  B -- Succeeds --> C[Serve fresh object]
  B -- Fails --> D{Stale window open?}
  D -- Open --> E[Serve stale, refresh later]
  D -- Closed --> F[No cached answer]
```
