...
A function runs when a rule invokes it. **Functions** stores your code. A function instance binds that code to an application or a firewall and carries its arguments, and [Rules Engine](/en/documentation/build/applications/reference/rules-engine/) decides which requests trigger it. [Azion Runtime](/en/documentation/runtime/overview/) then executes one handler on the Azion edge node that received the request. The execution context has a fixed memory and CPU budget.

The sections that follow trace that behavior in order: the request path, function instances, handlers and the event model, application functions against firewall functions, framework builds through Azion Bundler, function inputs, and execution limits.
...
The code itself is capped where you write it: 6 MB through [Azion Console](https://console.azion.com/) and 20 MB through the [API](https://api.azion.com/).
