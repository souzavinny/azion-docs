...
```mermaid
flowchart TD
  Client([Client]) -->|HTTP request| Node[Edge node receives the request]
  Node --> FwRule{Firewall rule with Run Function}
  FwRule -->|Criteria match| FwHandler[The firewall handler runs on Azion Runtime]
  FwHandler -->|"ctx.deny() called"| Blocked([Request blocked])
  FwHandler -->|Returns without deny| AppRule{Application rule with Run Function}
  FwRule -->|No match| AppRule
  AppRule -->|Criteria match| Fetch[The fetch handler runs on Azion Runtime]
  AppRule -->|No match| NoFn([No function runs])
  Fetch --> Response([Response returned to the client])
```
...
The application's Rules Engine then applies its own rules, and a **Run Function** rule whose criteria match runs the `fetch` handler of the selected instance. That handler returns a `Response` to the client.
...
[Azion Runtime](/en/documentation/runtime/overview/) calls your code through a fixed entry point. A function therefore ships as an ES module whose default export is an object with named handlers: `fetch`, which answers a request, and `firewall`, which decides whether a request may continue. The runtime calls the handler that matches the event and passes it the request.
...
```mermaid
flowchart LR
  Project[Framework project] --> Bundler[Azion Bundler]
  Bundler --> Fn[Generated functions in JavaScript or WebAssembly]
  Fn --> Runtime[Azion Runtime]
  Runtime --> DC[Azion data centers]
```
