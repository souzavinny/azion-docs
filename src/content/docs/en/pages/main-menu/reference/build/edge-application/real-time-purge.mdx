...
- **Tiered Cache**: purges cache from the second layer at the edge. Can only be done when the [Tiered Cache](/en/documentation/build/applications/cache/tiered-cache/) module is active.
...
Purges cached objects by passing a list of [cache keys](/en/documentation/build/applications/cache/#cache-keys) as an argument. 
...
- Image format according to browser support when using [Image Processor](/en/documentation/build/applications/image-processor/).
...
You can purge objects with content variation based on cookies or query strings using the **Advanced Cache Key** feature from the [Application Accelerator](/en/documentation/build/applications/application-accelerator/) module. 


To remove objects with [cookie-based variation](/en/documentation/build/applications/cache/#advanced-cache-key), you can use a purge by cache key, entering all variations individually, or a purge by wildcard, using `@@*` at the end.

To purge objects with [variation based on query string](/en/documentation/build/applications/cache/#advanced-cache-key), you can use: 
...
You can purge images that are treated by [Image Processor](/en/documentation/build/applications/image-processor/).
...
By default, Azion allows caching of `GET` and `HEAD` methods only. You can enable `POST` or `OPTIONS` caching with the [Application Accelerator](/en/documentation/build/applications/application-accelerator/#support-for-http-methods) module.
