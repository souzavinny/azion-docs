...
permalink: /documentation/build/applications/reference/real-time-purge/
---
...
- **Tiered Cache**: purges cache from the second layer at the edge. Can only be done when the [Tiered Cache](/en/documentation/build/applications/reference/cache/tiered-cache/) module is active.
...
Purges cached objects by passing a list of [cache keys](/en/documentation/build/applications/reference/cache/#cache-keys) as an argument. 
...
- Cookies when using [Advanced Cache Key](/en/documentation/build/applications/reference/cache-settings/#advanced-cache-key).
- Image format according to browser support when using [Image Processor](/en/documentation/build/applications/reference/image-processor/).
...
You can purge objects with content variation based on cookies or query strings using the **Advanced Cache Key** feature from the [Application Accelerator](/en/documentation/build/applications/reference/application-accelerator/) module. 


To remove objects with [cookie-based variation](/en/documentation/build/applications/reference/cache/#advanced-cache-key), you can use a purge by cache key, entering all variations individually, or a purge by wildcard, using `@@*` at the end.

To purge objects with [variation based on query string](/en/documentation/build/applications/reference/cache/#advanced-cache-key), you can use: 
...
You can purge images that are treated by [Image Processor](/en/documentation/build/applications/reference/image-processor/).
...
If you use the [Large File Optimization](/en/documentation/build/applications/reference/cache-settings/#large-file-optimization) feature in your application, large files might still remain in cache. To perform a purge in this case, you must list the cache keys for each fragment individually.
...
By default, Azion allows caching of `GET` and `HEAD` methods only. You can enable `POST` or `OPTIONS` caching with the [Application Accelerator](/en/documentation/build/applications/reference/application-accelerator/#support-for-http-methods) module.
