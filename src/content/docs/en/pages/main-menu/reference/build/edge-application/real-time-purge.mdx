...
However, if specific cached objects must be removed from the cache before they expire, you have the option to purge cache with Real-Time Purge. Regular cache purging as part of your application helps you:
...
- **Cache**: purges cache kept on Azion edge nodes.
- **Tiered Cache**: purges cache from the second layer, between the edge nodes and the origin. Can only be done when the [Tiered Cache](/en/documentation/build/cache/tiered-cache/) module is active.
...
- Cookies when using [Advanced Cache Key](/en/documentation/build/cache/reference/cache-settings/#advanced-cache-key).
...
If you use the [Large File Optimization](/en/documentation/build/cache/reference/cache-settings/#large-file-optimization) feature in your application, large files might still remain in cache. To perform a purge in this case, you must list the cache keys for each fragment individually.
...
The Real-Time Purge limits, including the purge operations that vary by support plan, are listed in [Cache limits](/en/documentation/build/cache/limits/).
