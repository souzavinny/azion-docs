...
permalink: /documentation/secure/workloads/guides/point-domain-to-azion/
...
There are separate instructions for the [legacy Domains](/en/documentation/build/applications/reference/domains/) settings and the new [Workloads](/en/documentation/secure/workloads/) product.

Before you point your traffic to Azion, you can test your custom domain on the edge locally by [staging your application through the hosts file](/en/documentation/build/applications/guides/stage-applications-through-hosts-file/).

:::note
This process will require you to associate a CNAME record to your domain. To associate an ANAME record to your application, [migrate your nameservers to Azion](/en/documentation/secure/workloads/guides/migrate-ns-to-azion/).
:::
...
1. Run the following `GET` request to retrieve your workload's ID, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/):
...
4. Run a `PATCH` request, , replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and adding the `workload_id` as a path, to update the `domains` field with the domains you wish to configure:
...
:::tip
If your application uses HTTPS, you might want to combine this process with the [association of a digital certificate](/en/documentation/secure/workloads/guides/create-a-digital-certificate/) to your custom domain.
:::
...
1. Run the following `GET` request to retrieve your domain's ID, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/):
...
4. Run a `PATCH` request, , replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and adding the `domain_id` as a path, to update the `cnames` field with the domains you wish to configure:
...
:::note
Some services don't allow you to change your ANAME resolution directly. You can point your custom ANAME record to your application by [migrating your nameservers to Azion](/en/documentation/secure/workloads/guides/migrate-ns-to-azion/).
:::
...
Note that there might be a delay in propagation time when you access your application. If that's the case, [you can run the dig command](/en/documentation/secure/edge-dns/guides/run-the-dig-command/) in your terminal to check the DNS records of your edge node. For example:
