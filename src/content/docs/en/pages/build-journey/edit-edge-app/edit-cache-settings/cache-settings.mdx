...
permalink: /documentation/build/applications/guides/tune-cache-settings/
---

import Tabs from '~/components/tabs/Tabs'


Leverage the capabilities of [Cache](/en/documentation/build/applications/reference/cache-settings/) to configure your application's [cache policies](/en/documentation/build/applications/reference/cache-settings/), including expiration values at the edge or browser level, large file optimization to break large files into smaller chunks, and cache key segmentation based on HTTP methods, query strings, cookies, or device groups.

:::caution[warning]
Some cache features require the [Application Accelerator](/en/documentation/build/applications/reference/application-accelerator/) module to be active. If **Application Accelerator** is activated, data transfer could generate usage-related costs. Check the [pricing page](/en/documentation/platform/pricing/) for more information.
:::

---

When you first [create an application](/en/documentation/get-started/start-with-a-template/), a cache setting variable will be created and activated by default. This guide will show you how to create and activate a brand new cache setting instance.
...
:::note
The maximum and default TTL values for the edge should be between `60` and `31536000` seconds (1 year). If you have the [Application Accelerator](/en/documentation/build/applications/reference/application-accelerator/) module activated, you can set this value to a minimum of `0`. Additionally, if you have the [Cache](/en/documentation/build/applications/reference/cache/tiered-cache/) module activated and wish to maintain cache in the cache layer, you must set TTL to at least `3` seconds.

A TTL zero isn't the same as a cache bypass. For more details, read about the difference between these settings on the [Bypass Cache behavior](/en/documentation/build/applications/reference/rules-engine/#bypass-cache) documentation.
:::
...
Instead of downloading a large content file and risking timeouts or connection terminations, [files can be fragmented](/en/documentation/build/applications/reference/cache-settings/#large-file-optimization) into smaller pieces and cached on-demand.
...
This section lets you customize how cache varies based on HTTP methods, query string fields, cookies and device groups through [cache keys](/en/documentation/build/applications/reference/cache-settings/#advanced-cache-key). You can choose to segment cache keys using these attributes to control how objects are stored and served from the cache.

:::caution[warning]
To enable all Application Accelerator functionalities, you must activate the [Application Accelerator](/en/documentation/build/applications/reference/application-accelerator/) module.
:::
...
The cache settings page now lists the newly created instance. However, this new cache setting isn't active in your application. You need to define what will trigger the implementation of the cache policies in your application. For this, you can use your application's [Rules Engine](/en/documentation/build/applications/reference/rules-engine/).
...
To verify how your content is being cached, you can [verify application cache indicators using Modheader for Google Chrome](/en/documentation/build/applications/guides/check-page-cache-time/).
</Fragment>

<Fragment slot="panel.api">
1. Run the following `PATCH` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and the `<application_id>` variable with [your application ID](/en/documentation/build/applications/guides/configure-main-settings/) to activate the [Application Accelerator](/en/documentation/build/applications/reference/application-accelerator/) module:
...
3. If you want to configure adaptive delivery for one of your [device groups](/en/documentation/build/applications/guides/create-device-groups/), run the following `GET` request beforehand:
...
5. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/), the `<application_id>` variable with [your application ID](/en/documentation/build/applications/guides/configure-main-settings/), and the `<device_group_id>` with the ID of the device group from the previous response, if needed:
...
| `modules.edge_cache.stale_cache.enabled` | Enables [stale cache](https://www.azion.com/en/documentation/build/applications/reference/cache-settings/#stale-cache) to serve expired content temporarily while fetching updated content from the origin. |
| `modules.edge_cache.large_file_cache.enabled` | Enables [large file optimization](https://www.azion.com/en/documentation/build/applications/reference/cache-settings/#large-file-optimization) by caching large files in fragments. |
...
7. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/), the `<application_id>` variable with [your application ID](/en/documentation/build/applications/guides/configure-main-settings/), and the `<cache_setting_id>` variable with the cache setting ID received in the response:
