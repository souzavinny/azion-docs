...
permalink: /documentation/build/applications/guides/work-with-origins/
...
There are separate instructions for API v3 using [legacy Origin](/en/documentation/build/applications/reference/domains/) settings and API v4 using the new [Connectors](/en/documentation/secure/connectors/) product.
...
:::tip
To implement the load balancing algorithm for multiple origins, see the [guide on multiple origins with Load Balancer](/en/documentation/build/applications/guides/multiple-origins/).
:::
...
When you [create an application](/en/documentation/get-started/start-with-a-template/), a default origin is created and activated automatically. This guide will show you how to create and activate a new origin with different configurations without removing or editing the default origin.
...
:::tip
To implement load balancing algorithm for multiple origins, see the [guide on multiple origins with Load Balancer](/en/documentation/build/applications/guides/multiple-origins/).
:::
...
:::note
The `${uri}` variable may already be selected by default if you didn't activate **Application Accelerator**. For more information on Applications modules, see the [Applications documentation](/en/documentation/build/applications/#modules).
:::
...
1. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/) and the `<application_id>` variable with [your application ID](/en/documentation/build/applications/guides/configure-main-settings/):
...
`} />
  ...
  | `type` | Sets the new Connector type to `http`. For more information on load balancing, check [Work with multiple origins](/en/documentation/build/applications/guides/multiple-origins/). |
...
`} />
  ...
  | --- | --- |
  | `name` | Sets the string in value as a name of the origin. |
  | `origin_type` | Sets the new origin type to `single_origin`. For more information on load balancing, check [Work with multiple origins](/en/documentation/build/applications/guides/multiple-origins/). |
  | `addresses` | Takes a list of objects for each address of the origin. Since this is an entry of the single origin type, you may only send one object with the `address` value in the array. |
...
3. Run the following `POST` request in your terminal, replacing `[TOKEN VALUE]` with your [personal token](/en/documentation/account/guides/personal-tokens/), the `<application_id>` variable with [your application ID](/en/documentation/build/applications/guides/configure-main-settings/), and the `<origin_id>` variable with the origin ID from when you [created a new origin via API](#via-api):
